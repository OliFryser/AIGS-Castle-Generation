...
from MapElites.MapElitesPlotter import PlotRecord
...
class FitnessRecord:
    def __init__(self, individual: InstructionTree, eval):
        self.individual = individual
        self.fitness = eval(individual)
...
class ConventionalEvolution(MapElites):
    ...
    def runCE(self, populationSize: int):
        outerTimer = Timer(
            f"conventional for {self.iterations} genrations, population {populationSize}",
            forcePrint=True,
        )
        outerTimer.start()
        ...
        selection = sorted(
            population,
            key=lambda individual: self.runSimulationCE(simulation, individual),
        )[:selectionSize]
        ...
        for i in range(self.iterations):
            ...
            for individual in population:
                records.append(
                    FitnessRecord(
                        individual, lambda i: self.runSimulationCE(simulation, i)
                    )
                )
            ...
            for y in range(0, 10):
                ...
                if i < len(population):
                    ...
                    entry = ArchiveEntry(
                        fitness,
                        Behaviors(
                            Behavior(x, "Conventional"), Behavior(y, "Evolution")
                        ),
                        population[i],
                    )
