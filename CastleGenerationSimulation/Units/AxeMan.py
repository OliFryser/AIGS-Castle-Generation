...
class AxeMan(Unit):
    def __init__(
        self, *args, health: int = 100, speed: float = 0.5, size=0.3, **kwargs
    ):
    ...
    def initFSM(self):
        ...
        demolishFsm.addTransition(goToGoalFSM, State.DEMOLISH, self.closeEnoughToBlock)
        demolishFsm.addTransition(
            State.DEMOLISH,
            State.WAIT,
            self.onAttackCoolDown,
        ...
        fsm.addTransition(
            goToGoalFSM,
            demolishFsm,
            self.foundWallWeakPoint,
            (self.setNodeTargetAndPath, (), {}),
            onExit=(self.targetGoal, (), {}),
        )
        fsm.addTransition(
            State.WAIT,
            goToGoalFSM,
    ...
    def foundWallWeakPoint(self):
        ...
        return (
            ...
            and self.path[0].materialBlock.blocking
        )

    def closeEnoughToBlock(self):
        return (
            self.target is not None
            and self.position.distance_to(self.target)
            < self.size + self.blockAttackRange
        )
    ...
    def strikeWall(self):
        ...
        if (
            block is not None
            and self.position.distance_to(block.position)
            < self.size + self.blockAttackRange + 0.6
        ):
    ...
    def destroyCastleElement(self, node: Node):
        ...
        self.level.castleMap[int(node.position.z - 0.5)][int(node.position.x - 0.5)] = (
            None
        )
        self.nodeTarget = None
    ...
    def moveCostAdjust2(self, node: Node, edge: Edge):
        cost = edge.cost
        if (
            edge.node.unit is not None
            and edge.node.unit is not self
            and edge.node.unit in self.teamMates
        ):
            cost += 10
        """
        """
    ...
    def moveCostAdjust3(self, node, edge):
        ...
        if (
            edge["node"]["unit"] is not None
            and edge["node"]["unit"] is not self.getAsData()
            and edge["node"]["unit"] in list(u.getAsData() for u in self.teamMates)
        ):
            cost += 10
        """
        """
