...
class Unit:
    ...
    def isNotInMelee(self):
        ...
        if self.inMelee not in self.enemies:
            ...
            return True
        if (
            self.position.distance_to(self.inMelee.position)
            > self.attackRange + self.size
        ):
            self.inMelee = None
            return True
        return False
    ...
    def meleeAttack(self):
        if (
            self.inMelee is not None
            and self.position.distance_to(self.inMelee.position) < self.attackRange
        ):
            ...
            for tnode in frontNodes:
                if tnode.materialBlock is not None and tnode.materialBlock.blocking:
                    if newPosition.distance_to(tnode.position) < self.size * 4:
                        newDirection = (
                            direction + (self.position - tnode.position).normalize()
                        ).normalize()
    ...
    def getFrontNodes(self, direction):
        ...
        if (
            x + dx > self.level.width
            or x + dx < 0
            or y + dy > self.level.height
            or y + dy < 0
        ):
            return []
    ...
    def unitCost(self, node):
        if (
            node.unit is not None
            and node.unit is not self
            and node.unit not in self.enemies
        ):
            return 10
        return 0
